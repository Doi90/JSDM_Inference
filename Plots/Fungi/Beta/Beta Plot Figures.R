...
BayesComm.df <- read.csv("Beta_df_Fungi_BayesComm.csv")
BayesComm.df <- BayesComm.df[,-c(1,6:10)]
Pollock.df <- read.csv("Beta_Fungi_Pollock.csv")
Pollock.df <- Pollock.df[,-c(1,6:10)]
#Ovaskainen.df <- read.csv("Beta_Fungi_Ovaskainen2016.csv")
#Ovaskainen.df <- Ovaskainen.df[,-1]
OvaskainenNS.df <- read.csv("Beta_Fungi_Ovaskainen2016NS.csv")
OvaskainenNS.df <- OvaskainenNS.df[,-c(1,6:10)]
OvaskainenNS.df <- OvaskainenNS.df[! OvaskainenNS.df$coefficient %in% c("dc0", "ground0", "quality0"),]
OvaskainenNS.df$coefficient <- factor(OvaskainenNS.df$coefficient)
Clark.df <- read.csv("Beta_Fungi_Clark.csv")
Clark.df <- Clark.df[,-c(1,6:10)]
boral.df <- read.csv("Beta_Fungi_boral.csv")
...
df <- rbind(BayesComm.df, boral.df, Clark.df, OvaskainenNS.df, Pollock.df)
...
df$coefficient <- capitalize(df$coefficient)
...
df$Model <- factor(df$Model, levels = rev(c("MPR","HPR","LPR","DPR","HLR-NS")))
# levels(df$Model) <- c("MPR","LPR","DPR","HLR-NS")
#df$Model <- factor(df$Model, levels = rev(levels(df$Model)))
df$Coefficient <- factor(df$Coefficient, levels = c("Intercept","Bark","Dc1","Dc2",
                                                    "Dc3","Dc4","Dc5","Diam","Epi",
                                                    "Ground3","Ground4",
                                                    "Quality3","Quality4"))
