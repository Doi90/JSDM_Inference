...
BayesComm.df <- read.csv("Beta_df_Mosquito_BayesComm.csv")
BayesComm.df <- BayesComm.df[,-c(1,6:10)]
Pollock.df <- read.csv("Beta_Mosquito_Pollock.csv")
Pollock.df <- Pollock.df[,-c(1,6:10)]
Ovaskainen.df <- read.csv("Beta_Mosquito_Ovaskainen2016.csv")
Ovaskainen.df <- Ovaskainen.df[,-c(1,6:10)]
OvaskainenNS.df <- read.csv("Beta_Mosquito_Ovaskainen2016NS.csv")
OvaskainenNS.df <- OvaskainenNS.df[,-c(1,6:10)]
Clark.df <- read.csv("Beta_Mosquito_Clark.csv")
Clark.df <- Clark.df[,-c(1,6:10)]
boral.df <- read.csv("Beta_Mosquito_boral.csv")
...
df$coefficient <- rep(c("Intercept", "Depth (cm)", "Temperature (Celsius)", "Oxidation Reduction Potential (Mv)",
                        "Salinity (ppm)", "Water Crowfoot", "Rushes", "Filamentous Algae", "Emergent Grass",
                        "Ivy Leafed Duckweed", "Bulrushes", "Reeds", "Marestail", "Common Duckweed"),species.num)
...
df$Coefficient <- factor(df$Coefficient,
                         levels = c("Intercept","Bulrushes","Common Duckweed",
                                    "Depth (cm)","Emergent Grass","Filamentous Algae",
                                    "Ivy Leafed Duckweed","Marestail",
