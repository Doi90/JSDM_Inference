...
BC.Fungi <- ggplot(df, aes(x,y)) +
  ...
  geom_line() +
  # scale_x_continuous(breaks=quantiles) + 
  scale_fill_brewer(type = seq, palette = "YlGnBu", guide="none") +
  ylim(0,3.5) +
  xlim(-1,1) +
  labs(x = NULL, y = NULL) +
  geom_vline(xintercept = 0, lwd = 1)
...
BC.Mosquitos <- ggplot(df, aes(x,y)) +
  ...
  geom_line() +
  # scale_x_continuous(breaks=quantiles) + 
  scale_fill_brewer(type = seq, palette = "Purples", guide="none") +
  ylim(0,3) +
  xlim(-1,1) +
  labs(x = NULL, y = NULL) +
  geom_vline(xintercept = 0, lwd = 1)
...
plot.col.species <- grid.arrange(BC.Birds, BC.Butterflies, BC.Eucalypts, BC.Frogs, BC.Fungi, BC.Mosquitos,
