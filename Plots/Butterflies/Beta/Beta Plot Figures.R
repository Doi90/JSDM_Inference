...
BayesComm.df <- read.csv("Beta_df_Butterfly_BayesComm.csv")
BayesComm.df <- BayesComm.df[,-c(1,6:10)]
#Pollock.df <- read.csv("Beta_Frog_Pollock.csv")
#Pollock.df <- Pollock.df[,c(-1,-6)]
#Ovaskainen.df <- read.csv("Beta_Frog_Ovaskainen2016.csv")
#Ovaskainen.df <- Ovaskainen.df[,-1]
OvaskainenNS.df <- read.csv("Beta_Butterfly_Ovaskainen2016NS.csv")
OvaskainenNS.df <- OvaskainenNS.df[,-c(1,6:10)]
boral.df <- read.csv("Beta_Butterfly_boral.csv")
boral.df <- boral.df[,-c(1,6:10)]
Clark.df <- read.csv("Beta_Butterflies_Clark.csv")
...
df$coefficient <- rep(c("Intercept", "Climate", "Broad-leaved Woodland", "Coniferous Woodland",
                        "Calcareous Substrates"),species.num)
...
df$Coefficient <- factor(df$Coefficient, levels = c("Intercept","Broad-leaved Woodland",
