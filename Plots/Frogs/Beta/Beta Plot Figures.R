...
BayesComm.df <- read.csv("Beta_df_Frog_BayesComm.csv")
...
OvaskainenNS.df <- read.csv("Beta_Frog_Ovaskainen2016NS.csv")
OvaskainenNS.df <- OvaskainenNS.df[,-c(1,6:10)]
Clark.df <- read.csv("Beta_Frog_Clark.csv")
Clark.df <- Clark.df[,-c(1,6:10)]
boral.df <- read.csv("Beta_Frog_boral.csv")
...
df$coefficient <- rep(c("Intercept", "Pond Surface Area", "Vertical Wall", "Road Density"),species.num)
...
df$Coefficient <- factor(df$Coefficient, levels = c("Intercept", "Pond Surface Area",
                                                    "Vertical Wall", "Road Density"))
