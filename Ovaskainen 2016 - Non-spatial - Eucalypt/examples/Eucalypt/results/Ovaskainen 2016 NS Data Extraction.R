...
file.mean <- openxlsx::read.xlsx("beta_Eucalypt.xlsx", 1, colNames = TRUE)
file.lower <- read.xlsx("beta_Eucalypt.xlsx",5, colNames = TRUE)
file.upper <- read.xlsx("beta_Eucalypt.xlsx",6, colNames = TRUE)
beta <- read.csv("beta_Eucalypt.csv", header = FALSE)

# Load standardisation data

dataset_sd <- read.csv("Eucalypts_sd.csv")
...
write.csv(df,"Beta_Eucalypt_Ovaskainen2016NS.csv")
