function [nf,lambda,eta,alpha,psijh,delta] = update_nf(nf,ns,mcmc,np,nur,a2r,b2r,lambda,eta,alpha,psijh,delta)
    nr = length(nf);
    b0 = 1; b1 = 0.0005;
    ...
    for i1=1:nr
        ...
        if uu < prob
            if  mcmc > 20 && num == 0 && all(lind < 0.995)
                nf(i1) = nf(i1) + 1;
                lambda1(nf(i1),:) = zeros(ns,1);
                eta1(:,nf(i1))=normrnd(0,1,[np(i1),1]);
                alpha1(nf(i1)) = 1;
                psijh1(:,nf(i1)) = gamrnd(nur/2,2/nur,[ns,1]);
                delta1(nf(i1)) = gamrnd(a2r,1/b2r);
