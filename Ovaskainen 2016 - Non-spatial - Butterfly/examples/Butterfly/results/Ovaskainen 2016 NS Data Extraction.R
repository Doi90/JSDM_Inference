...
file.mean <- openxlsx::read.xlsx("beta_Butterfly.xlsx", 1, colNames = TRUE)
file.lower <- read.xlsx("beta_Butterfly.xlsx",5, colNames = TRUE)
file.upper <- read.xlsx("beta_Butterfly.xlsx",6, colNames = TRUE)
beta <- read.csv("beta_Butterfly.csv", header = FALSE)

# Load standardisation data

dataset_sd <- read.csv("Butterflies_sd.csv")
...
write.csv(df,"Beta_Butterfly_Ovaskainen2016NS.csv")
