...
file.mean <- openxlsx::read.xlsx("beta_Mosquito.xlsx", 1, colNames = TRUE)
file.lower <- read.xlsx("beta_Mosquito.xlsx",5, colNames = TRUE)
file.upper <- read.xlsx("beta_Mosquito.xlsx",6, colNames = TRUE)
beta <- read.csv("beta_Mosquito.csv", header = FALSE)

# Load standardisation data

dataset_sd <- read.csv("Mosquitos_sd.csv")
...
write.csv(df,"Beta_Mosquito_Ovaskainen2016NS.csv")
