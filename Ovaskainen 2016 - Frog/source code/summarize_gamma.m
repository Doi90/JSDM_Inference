function [] = summarize_gamma(folder,dataset,quantiles,true_values,gammarep,covariates,traitnames,gammaT)
nt=length(traitnames);
nc=length(covariates);
resultsfolder=strcat(folder,'results\');
filename=strcat(resultsfolder,'gamma',dataset,'.xlsx');
warning('off','all');
for k=0:(length(quantiles)+true_values)
    if k==0
        vals=reshape(mean(gammarep),nt,nc)';
    elseif k<=length(quantiles)
        vals=reshape(quantile(gammarep,quantiles(k)),nt,nc)';
    else
        vals=gammaT';
    end
    A = cell(nt+1,nc+1);
    A{1,1}='';
    for i=1:nt
        A{i+1,1}=traitnames{i};
    end
    for j=1:nc
        A{1,j+1}=covariates{j};
    end
    for i=1:nt
        for j=1:nc
            A{i+1,j+1}=vals(j,i);
        end
    end
    if k==0
        xlswrite(filename,A,1);
    elseif k<=length(quantiles)
        xlswrite(filename,A,num2str(quantiles(k)));
    else
        xlswrite(filename,A,'true values');
    end
end
warning('on','all');
