...
# load data
dat <- read.csv("Butterfly_Compiled.csv")

# convert fauna to presence/absence
fauna <- as.matrix(dat[, 1:55])

# covariates (scale is data set dependant)
preds <- as.matrix(dat[, 56:59])
#preds[, 1:4] <- scale(preds[, 1:4]) Don't need to scale these
...
preds.new <- preds
butterfly.pred <- predict(full, preds.new)
